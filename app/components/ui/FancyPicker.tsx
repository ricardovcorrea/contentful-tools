  ...
  return (
    <div ref={ref} className="relative">
      {compact ? (
        ...
        >
          {/* Logo area — bare image or initials pill */}
          <div
          ...
          >
            <LogoAvatar
              ...
              className={`h-5 rounded text-[9px] font-bold shrink-0 border ${accentClass}`}
              size={20}
            />
          </div>
        ...
        <button
          onClick={() => !disabled && setOpen((p) => !p)}
          className={`w-full flex flex-col items-start gap-1.5 px-3 py-2.5 rounded-xl border transition-all duration-150 ${
            disabled
              ? "cursor-wait opacity-50 border-gray-300 bg-gray-200/50"
              : open
        ...
        >
          ...
          <div className="w-full flex items-center justify-between gap-2">
            {/* Logo — bare wide image or initials pill */}
            <div className="flex items-center" style={{ height: 32 }}>
              <LogoAvatar
                assetId={selected?.imageAssetId}
                fallback={initials(selected?.label ?? value)}
                className={`h-8 rounded-lg text-[10px] font-bold shrink-0 border ${accentClass}`}
                size={32}
              />
            </div>
            <svg
              ...
              return (
                ...
                >
                  <LogoAvatar
                    ...
                    className={`h-6 rounded-md text-[9px] font-bold shrink-0 border ${
                      isActive
